...
function createIncoming(stream) {
  ...
  stream.incoming.once('header', function(header) {
    if (header.opcode < 0x03) {
      stream.emit('incoming', stream.incoming);

      return;
    }
    if (header.opcode < 0x07) {         //unknown non-control opcodes
      stream.emit('unknownincoming', stream.incoming);

      return;
    }
    
    if (header.opcode === 0x08) {      //supported control opcodes
      stream.source.end();

      return;
    }
    if (header.opcode === 0x09) {
        stream.emit('ping', stream.incoming);
        
        return;
    }
    if (header.opcode === 0x0a) {
        stream.emit('pong', stream.incoming);
        
        return;
    }
    
    if (header.opcode < 0x0f) {   //unknown control opcodes
      stream.emit('unknowncontrol', stream.incoming);
      
      return;
    }
    stream.emit('error', new Error('Cannot handle unsupported opcode.'));
  });
